...
WHERE seat_no = 'B1'
  ...
  AND f.departure_date::date = '2020-12-18';

SELECT '123'::integer; -- SELECT '123a'::integer -> error

SELECT interval '2 years 1 days'; -- 2 years 0 mons 1 days 0 hours 0 mins 0.0 secs
...
SELECT cast(now() - interval '2 days' AS TIME);

-- https://www.postgresql.org/docs/current/functions-datetime.html#FUNCTIONS-DATETIME-EXTRACT
SELECT EXTRACT(YEAR FROM now());
SELECT EXTRACT(MONTH FROM now());
SELECT EXTRACT(DAY FROM now());
SELECT INTERVAL '80 minutes'; -- 0 years 0 mons 0 days 1 hours 20 mins 0.0 secs
SELECT EXTRACT(MINUTES FROM INTERVAL '80 minutes'); -- 20

SELECT t2.count - t1.count cnt_free_seats
FROM (SELECT f.aircraft_id,
      ...
      GROUP BY f.aircraft_id) t1
         JOIN (SELECT aircraft_id,
                      count(*)
               FROM seat
               GROUP BY aircraft_id) t2 USING (aircraft_id);

SELECT EXISTS(SELECT 1 FROM ticket WHERE id = 2);
-- true если хоть одна запись существует

-- 2 variant Проще решать через пересечение множеств
SELECT s.seat_no
FROM seat s
WHERE aircraft_id = 1
...
SELECT first_value(f.arrival_date - f.departure_date)
       ...
       last_value(f.arrival_date - f.departure_date) over (ORDER BY (f.arrival_date - f.departure_date)) min_value,
       count(*) OVER ()
FROM flight f
         JOIN airport a ON a.code = f.arrival_airport_code
         JOIN airport d ON d.code = f.departure_airport_code
...
SELECT t.passenger_name,
       count(*),
       round(100.0 * count(*) / (SELECT count(*) FROM ticket), 2) percent
FROM ticket t
...
SELECT t1.*,
       COALESCE(lead(sum_cost) OVER (order by t1.sum_cost), t1.sum_cost) - t1.sum_cost diff
